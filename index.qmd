...
# Parte III - Experimento Ideal e Fundamentos da Regressão {.unnumbered}
...
# Prática com dados simulados

Os três exercícios apresentados neste material são **hipotéticos e foram elaborados especificamente para fins didáticos**. Eles não reproduzem exemplos empíricos da obra de Angrist e Pischke.

O objetivo é mostrar, em uma sequência única, três situações:

1.  o modelo de resultados potenciais com seleção não aleatória;
2.  a eliminação do viés de seleção por atribuição aleatória;
3.  a análise de um experimento randomizado com regressão e um vetor de covariáveis relevantes.
...
# Parte IV - Fundamentos da Regressão {.unnumbered}
...
# Análise de Regressão com Microdados da PNADC

Este capítulo apresenta uma análise de regressão aplicada aos microdados da Pesquisa Nacional por Amostra de Domicílios Contínua (PNADC), do IBGE, referentes ao primeiro trimestre de 2023. O objetivo é investigar a relação entre rendimento do trabalho, escolaridade e sexo, partindo de estatísticas descritivas ponderadas pela amostra até a estimação de modelos de regressão linear com diagnóstico de resíduos e correção de heterocedasticidade.

O dicionário de variáveis da PNADC pode ser consultado em:
...
```{r}
#| label: download-pnadc

# Baixando a PNADC (1º trimestre de 2023, com deflator de renda)

pnadc_2023_1 <- get_pnadc(
  year = 2023,
  quarter = 1,
  deflator = TRUE
)
...
# Passando o objeto de amostra complexa (svy) para um data frame

pnadc2023_1_df <- pnadc_2023_1$variables

# saveRDS(pnadc2023_1_df, "pnadc_rds.rds")
```

## Estatísticas descritivas ponderadas

Antes de estimar os modelos de regressão, é importante conhecer a estrutura amostral e calcular estatísticas descritivas considerando o peso amostral (`V1028`).
...
# Contagem da população entrevistada por subgrupo,
# definido pela variável sexo (V2007)
...
```{r}
#| label: contagem-sexo-ponderada

# Estimativa da contagem da população (ponderada) por subgrupo,
# definido pela variável sexo (V2007)
...
```{r}
#| label: contagem-sexo-raca

# Estimativa da contagem populacional com interação entre variáveis
# categóricas de agrupamento - sexo + raça/cor
...
pnadc2023_1_df %>%
  count(V2010, V2007, wt = V1028)
```

### Médias ponderadas de variáveis contínuas

```{r}
#| label: media-renda

# Estimativa da média ponderada da renda

pnadc2023_1_df %>%
  summarise("media da renda" =
              weighted.mean(VD4020, w = V1028, na.rm = TRUE))

# teste - sem peso, para efeito de comparação

pnadc2023_1_df %>%
  summarise("media da renda" =
              mean(VD4020, na.rm = TRUE))
```

### Estatísticas ponderadas para subconjuntos de dados

```{r}
#| label: rhtp-brancos

# Estimativa do rendimento mensal habitual do trabalho principal
# (VD4016) para pessoas brancas (V2010) com 14 anos ou mais de idade

pnadc2023_1_df %>%
  filter(V2010 == "Branca") %>%
  summarise("RHTP" = weighted.mean(VD4016, w = V1028, na.rm = TRUE))
```

```{r}
#| label: rhtp-pretos-pardos

# Estimativa do rendimento mensal habitual do trabalho principal
# (VD4016) para pessoas pretas ou pardas (V2010) com 14 anos ou mais de idade
...
# Variável de rendimento efetivo de todos os trabalhos,
# para pessoas com 14 anos ou mais

summary(pnadc2023_1_df$VD4020)
```

```{r}
#| label: hist-renda-1000

# Histograma com largura de classe de R$ 1.000

ggplot(pnadc2023_1_df,
       aes(x = VD4020), weight = V1028) +
  geom_histogram(binwidth = 1000, fill = "blue", color = "white") +
  xlim(0, 30000)
```

```{r}
#| label: hist-renda-100

# Histograma com largura de classe de R$ 100
...
```{r}
#| label: scatter-escolaridade-renda

# Anos de escolaridade vs. rendimento

p <- ggplot(pnadc2023_1_df, aes(x = VD3005, y = VD4020)) +
  geom_point() +
  labs(x = "Anos escol.", y = "Salário") +
  theme_minimal()

print(p)
```

## Tratamento das variáveis para a regressão

### Anos de estudo (VD3005) em formato numérico

A variável de escolaridade (`VD3005`) vem originalmente como categórica (texto). Para uso em modelos de regressão, ela é recodificada para uma escala numérica sequencial de anos de estudo.

```{r}
#| label: recod-escolaridade

# vendo os valores originais
unique(pnadc2023_1_df$VD3005)

pnadc2023_1_df <- pnadc2023_1_df %>%
  mutate(VD3005 = case_when(
...
# transformando em formato numérico
pnadc2023_1_df$VD3005 <- as.numeric(pnadc2023_1_df$VD3005)

# checando
typeof(pnadc2023_1_df$VD3005)

gc()
```

### Salário-hora (SAL_HORA)

O rendimento também tem relação com o número de horas trabalhadas. Para captar essa dimensão, foi criada a variável `SAL_HORA`, obtida pela divisão do rendimento mensal efetivo do trabalho principal (`VD4017`) pelas horas normalmente trabalhadas no trabalho principal (`V4039`).

```{r}
#| label: sal-hora

# 1) horas normalmente trabalhadas no trabalho principal: V4039
# 2) rendimento mensal efetivo no trabalho principal: VD4017

pnadc2023_1_df$SAL_HORA <- pnadc2023_1_df$VD4017 / pnadc2023_1_df$V4039
```
...
ggplot(pnadc2023_1_df, aes(x = V4039)) +
  geom_histogram(binwidth = 1, color = "black", fill = "blue", alpha = 0.6) +
  theme_minimal() +
  xlim(0, 80)
```
...
pnadc2023_1_df$dummy_sexo <- ifelse(pnadc2023_1_df$V2007 == "Mulher", 1, 0)
```

## Modelos de regressão

### Modelo inicial: rendimento e escolaridade (sem peso amostral)

Como primeira aproximação, estima-se um modelo linear simples do rendimento (`VD4020`) em função dos anos de escolaridade (`VD3005`), sem uso do peso amostral.

```{r}
#| label: modelo-inicial

modelo_inicial <- lm(VD4020 ~ VD3005,
                      data = pnadc2023_1_df,
...
### Modelo 1: log(salário-hora) e escolaridade

Substitui-se a variável dependente pelo logaritmo do salário-hora (`SAL_HORA`), forma funcional mais adequada para variáveis de rendimento.

```{r}
#| label: modelo1

modelo1 <- lm(log(SAL_HORA) ~ VD3005,
              data = pnadc2023_1_df %>%
                filter(SAL_HORA != 0))
...
#### Diagnóstico dos resíduos — Modelo 1

```{r}
#| label: resid-modelo1

resid_modelo1 <- modelo1$residuals

plot(density(resid_modelo1, kernel = c("gaussian")),
     main = "Resíduos (fdp)",
     xlim = c(-3, 3))
```

```{r}
#| label: resid-plot-modelo1

plot(modelo1$residuals, col = 2)
```

```{r}
#| label: qqplot-modelo1

# QQ plot
qqnorm(resid_modelo1)
qqline(resid_modelo1, col = "red")
```

```{r}
#| label: bptest-modelo1
...
```{r}
#| label: robusto-modelo1

# Calculando erros padrão robustos à heterocedasticidade (White)

ep_robusto <- vcovHC(modelo1, type = "HC1")
ep_robusto

# Refazendo os testes t e IC com os erros padrão robustos
coeftest(modelo1, vcov. = ep_robusto)
confint(modelo1, vcov. = ep_robusto)
```

### Modelo 2: incluindo dummy de sexo

```{r}
#| label: modelo2

modelo2 <- lm(log(SAL_HORA) ~ VD3005 + dummy_sexo,
              data = pnadc2023_1_df %>%
                filter(SAL_HORA != 0))
...
#### Diagnóstico dos resíduos — Modelo 2

```{r}
#| label: resid-modelo2

resid_modelo2 <- modelo2$residuals

plot(density(resid_modelo2, kernel = c("gaussian")),
     main = "Resíduos (fdp)",
     xlim = c(-2, 2))
```

```{r}
#| label: resid-plot-modelo2

plot(modelo2$residuals, col = 2)
```

```{r}
#| label: qqplot-modelo2

# QQ plot
qqnorm(resid_modelo2)
qqline(resid_modelo2, col = "red")
```

```{r}
#| label: bptest-modelo2
...
```{r}
#| label: robusto-modelo2

# Calculando erros padrão robustos à heterocedasticidade

ep_robusto2 <- vcovHC(modelo2, type = "HC1")
ep_robusto2

# Refazendo os testes t e IC com os erros padrão robustos
coeftest(modelo2, vcov. = ep_robusto2)
confint(modelo2, vcov. = ep_robusto2)
```

### Modelo 3: escolaridade, sexo e efeito de interação

```{r}
#| label: modelo3

modelo3 <- lm(log(SAL_HORA) ~ VD3005 + dummy_sexo + dummy_sexo * VD3005,
              data = pnadc2023_1_df %>%
                filter(SAL_HORA != 0))
...
#### Diagnóstico dos resíduos — Modelo 3

```{r}
#| label: resid-modelo3

resid_modelo3 <- modelo3$residuals

plot(density(resid_modelo3, kernel = c("gaussian")),
     main = "Resíduos (fdp)",
     xlim = c(-3, 3))
```

```{r}
#| label: resid-plot-modelo3

plot(modelo3$residuals, col = 2)
```

```{r}
#| label: qqplot-modelo3

# QQ plot
qqnorm(resid_modelo3)
qqline(resid_modelo3, col = "red")
```

```{r}
#| label: bptest-modelo3
...
```{r}
#| label: robusto-modelo3

# Calculando erros padrão robustos à heterocedasticidade (White)

ep_robusto3 <- vcovHC(modelo3, type = "HC1")
ep_robusto3

# Refazendo os testes t e IC com os erros padrão robustos
coeftest(modelo3, vcov. = ep_robusto3)
confint(modelo3, vcov. = ep_robusto3)
```

### Comparação dos modelos

A tabela a seguir resume, lado a lado, os três modelos estimados, permitindo comparar os coeficientes de escolaridade, sexo e do termo de interação entre os modelos.

```{r}
#| label: stargazer-comparacao
#| results: asis
...
Como extensão da análise, sugere-se incluir a variável raça/cor/etnia (`V2010`) nos modelos acima, avaliando seu efeito sobre o log do salário-hora, isoladamente e em interação com escolaridade e sexo.

```{r}
#| label: exercicio-raca
#| eval: false

# Espaço reservado para a inclusão da variável raça/cor/etnia (V2010)
...
Gujarati, D. N. (2006). Econometria básica (4ª ed.; M. J. C. Monteiro, Trad.). Elsevier.

Hanck, C., Arnold, M., Gerber, A., & Schmelzer, M. (2018). Introduction to econometrics with R. https://www.econometrics-with-r.org/

Hoffmann, R. (2006). Análise de regressão: Uma introdução à econometria. Hucitec.

Kerns, G. J. (2018). Introduction to probability and statistics using R (3rd ed.). https: //ipsur.org/
...
Vasconcellos, M. A. S. de, & Alves, D. C. de O. (Eds.). (2000). Manual de econometria: Nível intermediário. Atlas.
